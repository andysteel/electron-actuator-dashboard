...
import { faCog, faChartLine, faArrowLeft, faCheckCircle,
faBug, faBan, faExclamationCircle, faEye } from '@fortawesome/free-solid-svg-icons';
...
export class ChartsComponent implements OnInit {
  ...
  private getMemoryManagement() {
    this.dashboardService.getMemoryUsed(this.application.monitoringUrl).subscribe(
      ...
      (error: HttpErrorResponse) => {
        ...
        this.messageEvent.emit({key: 'messages', severity: 'error', life: 4000, summary: `${this.errorMessage}`});
      }
    )
  }
  ...
  private initializeMemoryBarChart() {
    ...
    private processTraces(traces: any) {
      this.traces = traces.filter((trace: any) => {
    ...
    private getTraces() {
      this.dashboardService.getHttpTraces(this.application.monitoringUrl).subscribe(
        (response: any) => {
          ...
          this.items = [
            {
              label:`Status: ${this.systemHealth?.status}`
            },
            {
              label: `DB: ${this.systemHealth?.components?.db?.details?.database} - ${this.systemHealth?.components?.db?.status}`
            },
            {
              label: `Disk Space: ${this.freeDiskSpace}`
            },
            {
              label: `Processors: ${this.processors}`
            },
            {
              label: `Up Time: ${this.processUpTime}`
            }
          ]
